...
# ── Config ────────────────────────────────────────────────────────────────────
BOT_TOKEN = "8174796687:AAEOHsrfAbKowu-p4qSaugmea6xeTaXh-BQ"
...
def commodity_kb():
    ...
    buttons = [InlineKeyboardButton(label, callback_data=f"commodity_{key}") for key, label in labels.items()]
    kb.add(*buttons)
    kb.add(InlineKeyboardButton("🏠 Main Menu", callback_data="menu_main"))
    return kb
...
def cmd_stock(msg):
    ...
    bot.send_message(msg.chat.id, "📊 Enter stock symbol (e.g. `AAPL`, `TSLA`):", parse_mode="Markdown")

@bot.message_handler(commands=["crypto"])
def cmd_crypto(msg):
    bot.send_message(msg.chat.id, "💰 *Crypto* — Choose mode:", parse_mode="Markdown", reply_markup=crypto_mode_kb())

@bot.message_handler(commands=["forex"])
def cmd_forex(msg):
    user_state[msg.from_user.id] = "await_forex_base"
    bot.send_message(msg.chat.id, "💱 Enter base currency (e.g. `USD`, `EUR`, `NGN`):", parse_mode="Markdown")

@bot.message_handler(commands=["news"])
def cmd_news(msg):
    user_state[msg.from_user.id] = "await_news"
    bot.send_message(msg.chat.id, "📰 Enter a news topic (e.g. `bitcoin`, `AI`, `nigeria`):", parse_mode="Markdown")
...
def handle_callback(call):
    ...
    if data == "menu_main":
        ...
        bot.send_message(cid, "🏠 *Main Menu* — What do you want to scrape?", parse_mode="Markdown", reply_markup=main_menu_kb())

    elif data == "menu_stock":
        user_state[uid] = "await_stock"
        bot.send_message(cid, "📊 Enter stock symbol (e.g. `AAPL`, `TSLA`):", parse_mode="Markdown")

    elif data == "menu_crypto":
        bot.send_message(cid, "💰 *Crypto* — Choose mode:", parse_mode="Markdown", reply_markup=crypto_mode_kb())

    elif data == "crypto_search":
        user_state[uid] = "await_crypto_search"
        bot.send_message(cid, "🔍 Enter coin id (e.g. `bitcoin`, `solana`, `ethereum`):", parse_mode="Markdown")

    elif data == "crypto_top":
        user_state[uid] = "await_crypto_top"
        bot.send_message(cid, "🏆 How many top coins? (e.g. `10`):", parse_mode="Markdown")

    elif data == "menu_forex":
        user_state[uid] = "await_forex_base"
        bot.send_message(cid, "💱 Enter base currency (e.g. `USD`, `EUR`, `NGN`):", parse_mode="Markdown")

    elif data == "menu_news":
        user_state[uid] = "await_news"
        bot.send_message(cid, "📰 Enter a news topic (e.g. `bitcoin`, `AI`, `nigeria`):", parse_mode="Markdown")
        ...
        try:
            ...
            bot.send_message(cid, fmt_commodity(result), parse_mode="Markdown", reply_markup=back_kb())
        ...
        try:
            ...
            bot.send_message(cid, fmt_forex(result), parse_mode="Markdown", reply_markup=back_kb())
        except Exception as e:
            bot.send_message(cid, f"❌ {e}", reply_markup=back_kb())
...
def handle_text(msg):
    ...
    if not state:
        bot.send_message(cid, "Use /menu or /start to begin.", reply_markup=main_menu_kb())
        return
    ...
    if state == "await_stock":
        ...
        try:
            ...
            bot.send_message(cid, fmt_stock(result), parse_mode="Markdown", reply_markup=back_kb())
        except Exception as e:
            bot.send_message(cid, f"❌ {e}", reply_markup=back_kb())
    ...
    # ── Crypto search ──
        ...
        try:
            ...
            bot.send_message(cid, fmt_coin(result), parse_mode="Markdown", reply_markup=back_kb())
        except Exception as e:
            bot.send_message(cid, f"❌ {e}", reply_markup=back_kb())
    ...
    # ── Forex base ──
        ...
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton("⏭ Skip — show all pairs", callback_data=f"forex_skip_target_{text.upper()}"))
        bot.send_message(
            cid,
